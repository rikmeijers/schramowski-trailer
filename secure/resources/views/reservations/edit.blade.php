...
@section('content')
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
            <h1 class="fw-bold mb-1">Reservierung bearbeiten</h1>
            <p class="text-body-secondary mb-0">Ändere Zeitraum, Status, Zahlung und Kundendaten.</p>
        </div>

        <div class="d-flex gap-2">
            <a href="{{ route('dashboard.reservation.show', $reservation->id) }}" class="btn btn-outline-secondary rounded-pill px-4">
    ...
    <div class="card ui-card ui-card-pad mt-3">
        <form method="POST" action="{{ route('reservations.update', $reservation->id) }}" class="row g-3"
            ...
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold"><i class="bi bi-truck me-1"></i> Anhänger</label>
                <select name="trailer_id" class="form-select @error('trailer_id') is-invalid @enderror">
                    <option value="">-- Anhänger auswählen --</option>
                    @foreach ($trailers as $trailer)
                        <option value="{{ $trailer->id }}" @selected(old('trailer_id', $reservation->trailer_id) == $trailer->id)>
            ...
            <div class="col-12 col-md-3">
                <label class="form-label fw-semibold"><i class="bi bi-calendar-event me-1"></i> Startdatum</label>
                <input type="date" name="start_date" value="{{ old('start_date', optional($reservation->start_date)->format('Y-m-d')) }}"
            ...
            <div class="col-12 col-md-3">
                <label class="form-label fw-semibold"><i class="bi bi-calendar-event me-1"></i> Enddatum</label>
                @php
                    $endInclusive = optional($reservation->end_date);
                @endphp
                <input type="date" name="end_date" value="{{ old('end_date', optional($endInclusive)->format('Y-m-d')) }}"
            ...
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold"><i class="bi bi-hourglass-split me-1"></i> Reservierungsstatus</label>
                <select name="status" class="form-select @error('status') is-invalid @enderror">
                    <option value="confirmed" @selected(old('status', $reservation->status) === 'confirmed')>Bestätigt</option>
                    <option value="pending" @selected(old('status', $reservation->status) === 'pending')>Pending (Telefon / noch nicht sicher)</option>
                </select>
                @error('status')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold"><i class="bi bi-credit-card me-1"></i> Zahlstatus</label>
                <select name="payment_status" class="form-select @error('payment_status') is-invalid @enderror" data-payment-status>
                    <option value="unpaid" @selected(old('payment_status', $reservation->payment_status) === 'unpaid')>Noch zu bezahlen</option>
                    <option value="partial" @selected(old('payment_status', $reservation->payment_status) === 'partial')>Teilweise bezahlt</option>
                    <option value="paid" @selected(old('payment_status', $reservation->payment_status) === 'paid')>Bezahlt</option>
                </select>
            ...
            <div class="col-12 col-md-6" data-partial-paid-wrapper style="display:none;">
                <label class="form-label fw-semibold"><i class="bi bi-cash-stack me-1"></i> Bereits bezahlt</label>
                <input type="tel" inputmode="decimal" pattern="[0-9]*[.,]?[0-9]{0,2}" step="0.01" min="0" name="partial_paid_amount"
                       value="{{ old('partial_paid_amount', $reservation->partial_paid_amount) }}"
            ...
            <div class="col-12">
                <div class="form-check">
                    ...
                    <input class="form-check-input" type="checkbox" name="ignore_buffer" id="ignore_buffer" value="1"
                           @checked(old('ignore_buffer', $reservation->ignore_buffer))>
            ...
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold"><i class="bi bi-hash me-1"></i> Kundennummer</label>
                <input name="customer_number" value="{{ old('customer_number', $reservation->customer_number) }}"
            ...
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold"><i class="bi bi-building me-1"></i> Firma (optional)</label>
                <input name="company_name" value="{{ old('company_name', $reservation->company_name) }}"
            ...
            <div class="col-12 col-md-6">
                ...
                <input name="customer_first_name" value="{{ old('customer_first_name', $reservation->customer_first_name) }}"
            ...
            <div class="col-12 col-md-6">
                ...
                <input name="customer_last_name" value="{{ old('customer_last_name', $reservation->customer_last_name) }}"
            ...
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold"><i class="bi bi-telephone me-1"></i> Telefonnummer</label>
                <input name="customer_phone" value="{{ old('customer_phone', $reservation->customer_phone) }}"
            ...
            <div class="col-12 col-md-6">
                <label class="form-label fw-semibold"><i class="bi bi-envelope me-1"></i> E-Mail</label>
            ...
            <div class="col-12">
                <div class="d-flex flex-column gap-2">
                    @php
                        // Determine selected service option from old input or existing reservation
                        if (old('service_option')) {
                            $selectedService = old('service_option');
                        } else {
                            if ($reservation->service_lehr) $selectedService = 'lehr';
                            elseif ($reservation->service_paket) $selectedService = 'paket';
                            else $selectedService = 'selber_beladen';
                        }
                    @endphp
            ...
            <div class="col-12">
                <label class="form-label fw-semibold"><i class="bi bi-journal-text me-1"></i> Notizen (optional)</label>
                <textarea name="notes" rows="3" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $reservation->notes) }}</textarea>
                @error('notes')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
